...
Endpoints:
  POST /api/cluster/similar     — 主查詢：找相似案例
...
class SimilarRequest(BaseModel):
    ...
    dimensions: list[str] = Field(
        ...
        description="要使用的維度（technical/institutional/industry/fundamental/attention）",
    )
    window: int = Field(default=20, ge=5, le=120, description="Window 天數（形態描述子計算區間）")
    ...
    regime_match: bool = Field(default=True, description="同 regime 限定模式 [CONVERGED]")
