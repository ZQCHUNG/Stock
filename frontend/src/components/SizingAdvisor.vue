...
import { NCard, NGrid, NGi, NTag, NSpace, NTooltip, NSpin, NInputNumber, NSlider } from 'naive-ui'
...
const lightIcon = computed(() => {
  ...
  if (data.value.light === 'green') return '●'
  if (data.value.light === 'yellow') return '●'
  return '●'
})
  ...
  <NCard title="風險倉位顧問 (R80)" size="small" style="margin-bottom: 16px" :segmented="{ content: true }">
    ...
    <NSpin :show="loading" size="small">
      ...
      <template v-if="data">
        ...
        <!-- Results -->
        <NGrid :cols="4" :x-gap="8" :y-gap="8" style="margin-bottom: 8px">
          <NGi>
            <MetricCard
              title="建議張數"
