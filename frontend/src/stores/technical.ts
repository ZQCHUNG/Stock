...
export const useTechnicalStore = defineStore('technical', () => {
  ...
  async function loadRiskBudget(code: string) {
    const seq = _loadSeq
    try {
      const data = await analysisApi.riskBudget(code)
      if (seq !== _loadSeq) return
      riskBudget.value = data
    } catch {
      riskBudget.value = null
    }
  }

  async function loadSignalSummary(code: string) {
    const seq = _loadSeq
    try {
      const data = await analysisApi.signalStockSummary(code)
      if (seq !== _loadSeq) return
      signalSummary.value = data
    } catch {
      signalSummary.value = null
    }
  }
  ...
  async function loadFundamentals(code: string) {
    const seq = _loadSeq
    try {
      const data = await stocksApi.fundamentals(code)
      if (seq !== _loadSeq) return
  ...
  async function loadWinnerDna(code: string) {
    const seq = _loadSeq
    try {
      const data = await analysisApi.winnerDnaMatch(code)
      if (seq !== _loadSeq) return
      winnerDna.value = data
    } catch {
      winnerDna.value = null
    }
  }
